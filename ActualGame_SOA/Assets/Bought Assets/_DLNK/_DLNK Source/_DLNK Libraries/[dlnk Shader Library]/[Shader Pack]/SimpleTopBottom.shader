...
///////// Gloss:
                ...
                float gloss = 1.0 - saturate(lerp((_TopSmoothness*node_4283),(lerp( dot(_SmoothnessMap_var.rgb,float3(0.3,0.59,0.11)), _MetallicGlossMap_var.a, _MetalAlphaGloss )*_Glossiness*lerp(dot(_DetailAlbedoMap_var.rgb,float3(0.3,0.59,0.11)),node_3659,_DetailAffected)),_InvertTopBottom_var)); // Convert roughness to gloss
                float perceptualRoughness = saturate(lerp((_TopSmoothness*node_4283),(lerp( dot(_SmoothnessMap_var.rgb,float3(0.3,0.59,0.11)), _MetallicGlossMap_var.a, _MetalAlphaGloss )*_Glossiness*lerp(dot(_DetailAlbedoMap_var.rgb,float3(0.3,0.59,0.11)),node_3659,_DetailAffected)),_InvertTopBottom_var));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _SmoothnessMap_var = tex2D(_SmoothnessMap,TRANSFORM_TEX(i.uv0, _SmoothnessMap));
                float roughness = saturate(lerp((_TopSmoothness*node_4283),(lerp( dot(_SmoothnessMap_var.rgb,float3(0.3,0.59,0.11)), _MetallicGlossMap_var.a, _MetalAlphaGloss )*_Glossiness*lerp(dot(_DetailAlbedoMap_var.rgb,float3(0.3,0.59,0.11)),node_3659,_DetailAffected)),_InvertTopBottom_var));
